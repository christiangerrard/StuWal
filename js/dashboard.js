const grid = document.querySelector(".recurring-grid");
const countDisplay = document.querySelector(".recurring-count");
const selector = document.querySelector(".period-selector");

// Preset index untuk cell yang akan diberi warna (0-based index)
const filledIndexMap = {
  "This Month": [0, 4, 12, 20, 27, 28, 29], // 7 kotak sesuai gambar
  "Last Month": [2, 4, 8, 10, 12], // contoh berbeda
  "Last Year": [1, 5, 9, 11], // misal 4 bulan dalam 12
};

function generateGrid(period) {
  grid.innerHTML = "";

  // Hapus kelas khusus sebelumnya
  grid.classList.remove("last-year");
  ...
  if (period === "This Month") {
    ...
    total = 12;
    grid.classList.add("last-year"); // Tambah kelas khusus
  }

  const filledIndexes = filledIndexMap[period] || [];
  countDisplay.textContent = filledIndexes.length;
...
function renderLegend() {
  ...
  categories.forEach((cat) => {
    ...
    item.innerHTML = `
        ...
        <div>
          ...
          <span class="amount">Rp ${(cat.percentage * 10000).toLocaleString(
            "id-ID"
          )},-</span>
        </div>
      `;
  ...
  const totalNominal = categories.reduce(
    (sum, cat) => sum + cat.percentage * 10000,
    0
  );
...
function updateChart() {
  ...
  pieChart.data.datasets[0].backgroundColor = categories.map(
    (cat) => cat.color
  );
  pieChart.update();
}
